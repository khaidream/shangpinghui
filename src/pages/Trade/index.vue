<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul v-for="order in orderInfo.detailArrayList" :key="order.skuId" class="list clearFix">
          <li>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{userDefalutAddress.fullAddress}}</span>
        收货人：<span>{{userDefalutAddress.consignee}}</span>
        <span>{{userDefalutAddress.phoneNum}}</span>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$store.dispatch('trade/getUserAddresss')
      this.$store.dispatch('trade/getOrderInfo')
    },
    computed:{
      ...mapState('trade',{addressInfo:'address',orderInfo:'orderInfo',orderId1:'orderId'}),
      // 将来提交订单最终选中的地址
      userDefalutAddress(){
    ...
    methods:{
      ...
      async submitOrder(){
        ...
        // 其余的六个参数 
        let data = {
          consignee :this.userDefalutAddress.consignee, //最终收件人的名字 
          consigneeTel: this.userDefalutAddress.phoneNum, //最终收件人的电话
          deliveryAddress: this.userDefalutAddress.fullAddress, //最终收件人的地址
          paymentWay: "ONLINE", //支付方式
          orderComment: this.msg, // 买家的留言信息
          orderDetailList:this.orderInfo.detailArrrayList  //商品清单
