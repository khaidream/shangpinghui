            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li @click="changeOrder('1')" :class="{active:isOne}">
                  <a >综合<span v-show="isOne" class="iconfont" :class="{'icon-jiantou_xiangshang':isAsc,'icon-jiantou_xiangxia':isDesc}"></span></a>
                </li>
                <li @click="changeOrder('2')" :class="{active:isTwo}">
                  <a >价格<span v-show="isTwo" class="iconfont" :class="{'icon-jiantou_xiangshang':isAsc,'icon-jiantou_xiangxia':isDesc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      // 在发请求的之前带给服务器参数【searchParams参数发生变化有数值带给服务器】
      this.getData()
    },
    computed:{
      // mapGetters里面的写法：传递数组，因为getters计算是没有划分模块【home、search】
