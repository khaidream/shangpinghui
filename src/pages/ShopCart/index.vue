  ...
  export default {
    ...
    methods: {
      ...
      handler:throttle(async function(type,disNum,cart){
        // type为了区分这三个元素
        // disNum 代表 变化的量1 和 -1 input最终的个数
        // cart代表 点击的哪一个产品 他身上有id
        // 向服务器发请求，修改数量
        switch(type){
          case 'add':
            // 带给服务器变化量
      ...
      async updateChecked(cart,$event){
        // 根据接口文档 需要带给服务器的是 0|1
        try{
          await this.$store.dispatch('shopcart/updatedCheckedById',{skuId:cart.skuId,isChecked:$event.target.checked? '1':'0'})
          this.getData()
        }catch(error){
          console.log(error.message);
        }
    ...
    computed:{
      ...
      // 判断底部的复选框是否勾选【全部产品】
      isAllCheck(){
        // 遍历数组里面原理，只有全部元素isChecked属性都为1则返回为真
