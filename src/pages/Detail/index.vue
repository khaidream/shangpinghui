          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id" changepirce="0" :class="{active:spuSaleAttrValue.isChecked==1}" @click="changeActive(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)">{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
    ...
    methods: {
      ...
      async addShopCar(skuId){
        ...
        try{
         ...
         // 2：服务器存储成功 --- 进行路由跳转进行传递参数
        //  下面这种方法可以进行路由传递参数
        //  this.$router.push({path:'/addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum})
        // 一些简单的数据skuNum，通过query形式给路由组件传递过去
        // 产品信息的数据比较复杂:skuInfo，通过会话存储（不持久化）
